...
c ndns is a photon pdf; nothing to do
         continue
c 2 is a neutron, but assumes you have provided a neutron PDF
c one could alternatively assume a proton PDF and isospin transform u<>d          
      elseif(ih.eq.2) then
         ! The isospin transformation u(ubar) <> d(dbar)
         tmp=fx(1)  !d_p content
         fx(1)=fx(2) !replace d_n with u_p content
         fx(2)=tmp  !replace u_p with d_p content
         ! similarly for qbar content
         tmp=fx(-1)
         fx(-1)=fx(-2)
         fx(-2)=tmp
         !fx(1)
c 22 is a neutron and assumed one has directly provided neutron PDFs         
      elseif(ih.eq.22) then
         return
      else
