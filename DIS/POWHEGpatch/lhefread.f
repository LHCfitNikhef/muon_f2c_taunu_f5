...
c newer versions of LH files may also carry the rad_kinreg,rad_ubfailrad flags
         call compressBlanks(string)
         j=0
         do k=1,7
            m=index(string(j+1:),' ')
            j=j+m
         enddo
         if(string(j+1:) /= ' ') then
            read(string(j+1:),*) rad_kinreg,rad_ubfailrad
         endif
 111     continue
      endif
...
c     regular
 ...
 998  continue
      ...
      do k=1,len(string)
         if(string(k:k) /= ' ') then
            l=l+1
            string(l:l)=string(k:k)
         else
            if(l==0 .or. string(l:l)==' ') then
               cycle
            else
               l=l+1
               string(l:l)=' '
