...
c provide multiplicity of emitter in underlyng Born
      ...
      integer ipol                                 !debug IB: polarization
...
c
c ... IB: include polarized case
c         
         ap=tf*(x**2+(1-x)**2)*br_born(iub)*(1-x)*(1-ipol)
     &     +tf*(x**2-(1-x)**2)*br_born(iub)*(1-x)*(ipol)
...
c QED, consider emission by photon 
     ...
     &        *3d0*chargeofparticle(raflav)**2*(1-ipol)
     &     +(x**2-(1-x)**2)*br_born(iub)*(1-x)
     &        *3d0*chargeofparticle(raflav)**2*(ipol)       !not needed for DIS
