...
c imode = 1: force load
     ...
     1     rad_ncsiynormsmx,nlegborn-1,maxprocborn)
      ...
      call zerohistnorms
c whether bounds were loaded or not:
      loaded=.false.
      nubound=powheginput('nubound')
c number of slices of y interval wher to compute bounds      
      rad_nynorms=powheginput('#iymax')
      if(rad_nynorms.lt.0) rad_nynorms=1
c number of slices of csi interval wher to compute bounds      
...
c factor to globally increase bounds
      ...
      call newunit(iun)
c force generation: if called with imode=0 we force the computation of bounds
...
c See if the grid file exists already
     ...
     1           trim(rnd_cwhichseed)//'.dat',exist=lpresent)
...
c sequence of number files
            call seed2string(jfile,chseed)
            inquire(file=pwgprefix(1:lprefix)//'ubound-'//
     1           trim(chseed)//'.dat',exist=lpresent)
c This one is not present: skip it and go to the next
     ...
     1           trim(chseed)//'.dat',  status='old',iostat=ios)
...
c delete and deallocate the last mintwrapper framework,
     ...
     1        status='unknown')
         ...
         open(unit=iun,file=pwgprefix(1:lprefix)//'ubound-'
     1        //trim(rnd_cwhichseed)//'.dat',status='unknown')
      endif
...
cccccccccccccccccccccccccccccccc
      ...
      if(ptsq.gt.ptsqmin) then
         ...
         if(.not.refuse_pdf()) then
            ...
            sig=sig*kn_jacreal
!     Polarised dis:   check the born is non-zero
!     Unpolarised-dis: check the born is positive
            if ( (flg_polarised_dis        .and.   (born .eq. 0d0)) .or.
     $           ((.not.flg_polarised_dis) .and.   (born .le. 0d0)) )
     $           then
               if(verbose) then
               write(*,*) ' *****************************'
     ...
     $              =1,flst_bornlength(rad_ubornsubp)) 
     ...
     $              ,rad_ubornsubp)).ge.4)) then
            ...
            xnorm=abs(sig/born/pwhg_upperb_rad())
            if (debug) then
...
c      write(*,*) ' cosine decay angle', kn_cthdec
 ...
 100  format('[',3(d14.8,','),d14.8,'],')
      ...
      if(rnd_cwhichseed=='none') then
         ...
         open(iun,status='unknown',
     1    file='pwghistnorms-'//trim(rnd_cwhichseed)//'.top')
      endif   
...
c in FKS, for final state radiation, pdf always cancel in real/born
      ...
      if (flg_dis_isr) then
         if(st_mufact2.lt.2.or.kn_x2.gt.0.9) then
            refuse_pdf=.true.
            return
         endif
      else
      ...
      do k=1,2
         ...
         enddo
         if(fl.eq.0) then
            if(( flg_polarised_dis          .and. (abs(sn*suppfc(x)).gt.30*abs(pdf(0))) ) .or.
     $           ( (.not. flg_polarised_dis)  .and. (sn*suppfc(x).gt.30*pdf(0)) ) )
     $      then
               refuse_pdf=.true.
               return
            endif
         elseif(fl.eq.22) then
            if(sn*suppfc(x)*em_alpha .gt. 30*pdf(22)) then
               refuse_pdf=.true.
               return
            endif
         elseif(abs(fl)>0 .and. abs(fl)<=6) then
            if( (flg_polarised_dis          .and. (abs(pdf(0))*suppfc(x).gt.30*abs(pdf(fl)))) .or.
     $           ((.not. flg_polarised_dis) .and. (pdf(0)*suppfc(x).gt.30*pdf(fl))))
     $      then
               refuse_pdf=.true.
               return
            endif
         elseif(abs(fl)==11.or.abs(fl)==13.or.abs(fl)==15) then
            if(pdf(fl)<0  .or.
     1       pdf(22)*em_alpha*suppfc(x).gt.30*pdf(fl) ) then
               refuse_pdf=.true.
               return
            endif
         endif
...
c     set up the grids
         ...
         open(unit=iunstat,file=pwgprefix(1:lprefix)//'-'
     $        //trim(rnd_cwhichseed)//'-borngrid-stat.dat',status='unknown') 
